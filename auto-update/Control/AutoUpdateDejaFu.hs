...
--
-- @since 0.1.4
mkAutoUpdateWithModify :: MonadConc m => UpdateSettings m a -> (a -> m a) -> m (m a)
mkAutoUpdateWithModify us f = mkAutoUpdateHelper us (Just f)

mkAutoUpdateHelper :: forall m a . MonadConc m => UpdateSettings m a -> Maybe (a -> m a) -> m (m a)
mkAutoUpdateHelper us updateActionModify = do
    ...
    -- only RTS exceptions.
    mask_ $ void $ fork $ fillRefOnExit $ do
        ...
        let loop responseVar maybea = do
                ...
                a <- maybe (updateAction us) id (updateActionModify <*> maybea)
